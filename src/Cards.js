        ...
        <div className='main'>
                    ...
                    {products.map((product) => {
                        return (
                            <li key={product.id} className="cards_item">
                                <div className="card">
                                    <div className="card_image"><img alt={product.title} src={product.image} /></div>
            ...
            {showCart ? <div className='popup-wrapper'>
                <div className='popup'>
                    ...
                    <div>
                        {cart.length === 0 ? (
                            <p style={{ textAlign: 'center', padding: '2rem', fontSize: '1.2rem', color: '#666' }}>Your cart is empty</p>
                        ) : (
                            cart.map((item) => (
                                <div key={item.cartId} className='cart_items'>
                                    <img className='cart_image' src={item.image} alt={item.title} />
                                    <h4 style={{ margin: '10px 0', textAlign: 'center', color: '#333' }}>{item.title}</h4>
                                    <p style={{ fontSize: '1.2rem', fontWeight: '600', color: '#667eea' }}>Price: ${item.price.toFixed(2)}</p>
                                    <button className='delete-btn' onClick={() => removeFromCart(item.cartId)}>Remove</button>
                                </div>
                            ))
                        )}
                    </div>
